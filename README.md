...
Evidence is generated, not hand-written: `bun scripts/project_status.ts --write` (last generated 2026-09-11 at commit `de1e7ca`).
Counts below are read from the repository at generation time; CI and issue badges stay live.

- Migrations applied in order: `db/migrations` — **47** files
- Reviewed benchmark corpus: `reviewed-benchmark-v2` — **118 cases** (straightforward_answerable 20, exact_reference 18, recognized_disagreement 16, missing_context 16, evidence_absent 16, misleading_premise 16, conversation_followup 16; 82 tuning / 36 held-out)
...
| Reviewed benchmark suite + release comparison | Implemented — 7 families incl. conversation_followup (multi-turn) with tuning/held-out splits (live counts in the generated status block); multidimensional comparison reporting | `apps/api/src/eval/benchmarkCorpus.ts`, `scripts/seed_benchmark.ts`, `tests/benchmarkCorpus.test.ts` |
...
bun run db:migrate      # applies db/migrations in order
...
# optional: seed the official reviewed benchmark suite (see generated status block for counts)
...
bun test                # unit + integration tests (hermetic: no external LLM)
...
├── db/migrations/  # ordered SQL migrations (count in generated status block)
